...
export class DomainParserPreProcessor {
    domainCompletePattern = /^;;\s*!pre-parsing:\s*{\s*type:\s*"(command|nunjucks|jinja2|python)"\s*,\s*(command:\s*"([\w:\-/\\\. ]+)"\s*(,\s*args:\s*\[([^\]]*)\])?|data:\s*"([\w:\-/\\\. ]+)")\s*}/gm;
    ...
    process(templatedProblem: string, workingDirectory: string): string {
        ...
        this.domainCompletePattern.lastIndex = 0;
        let match = this.domainCompletePattern.exec(templatedProblem);
        if (match && match[1]) {
            switch (match[1]) {
                case "command":
